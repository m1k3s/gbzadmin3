...
class Player
{
    public:
        ...
        void change_to_hunter() {
            team = HunterTeam;
        }
        double duration(time_t now) {
            return difftime(now, time_joined);
        }

        void set_id(guint8 _id) {
            id = _id;
        }
        void set_type(guint16 _type) {
            type = _type;
        }
        void set_team(guint16 _team) {
            team = _team;
        }
        void set_wins(guint16 _wins) {
            wins = _wins;
        }
        void set_losses(guint16 _losses) {
            losses = _losses;
        }
        void set_tks(guint16 _tks) {
            tks = _tks;
        }
        void set_score(guint16 _wins, guint16 _losses) {
            wins = _wins;
            losses = _losses;
        }
        void set_callsign(Glib::ustring _callsign) {
            callsign = _callsign;
        }
        void set_motto(Glib::ustring _motto) {
            motto = _motto;
        }
        void set_time_joined(time_t _joined) {
            memcpy((time_t*)&time_joined, &_joined, sizeof(time_t));
        }
        ...
        void set_autoPilot(bool is_auto) {
            autopilot = is_auto;
        }
        void set_status(guint8 _status) {
            status = _status;
        }
        void set_registered(guint8 registered) {
            status |= registered;
        }
        void set_verified(guint8 verified) {
            status |= verified;
        }
        void set_admin(guint8 admin) {
            status |= admin;
        }
        void set_IP(Glib::ustring _ip) {
            IP = _ip;
        }
        void set_IP(struct in_addr addr);
        void set_flag(Glib::ustring _flag) {
            flag = _flag;
            flagged = true;
        }
        void clear_flag() {
        ...
        bool get_registered() {
            return ((status & IsRegistered) != 0);
        }
        bool get_verified() {
            return ((status & IsVerified) != 0);
        }
        bool get_admin() {
            return ((status & IsAdmin) != 0);
        }
        bool get_paused() {
            return paused;
        }
        ...
        Glib::ustring get_flag() {
            return flag;
        }
        gint get_type() {
            return type;
        }
        time_t get_time_joined() {
            return time_joined;
        }
